...
class EasyBlogModelBlog extends JModel
{
	 ...
	 *
	*/
	function getBlogsBy( $type, $typeId = 0, $sort = '', $max = 0 , $published = EBLOG_FILTER_PUBLISHED , $search = false, $frontpage = false, $excludeBlogs	= array(), $pending = false, $dashboard = false, $protected = true , $excludeCats = array() , $includeCats = array() , $postType = null )
	{
		$db		= JFactory::getDBO();
		$my		= JFactory::getUser();
		...
		if( $type != 'teamblog' && !$dashboard )
		{
			if( $config->get( 'main_includeteamblogpost' ) && !empty($teamBlogIds) )
			{
				...
				if( !empty( $jsEventPostIds ) )
				{
					$tmpIds = implode( ',', $jsEventPostIds);
					$queryWhere	.= ' AND (u.team_id IN ('.$teamBlogIds.') OR a.id IN (' . $tmpIds . ') OR a.`issitewide` = ' . $db->Quote('1') . ')';
				}
...
// 		}
	...
	public function getBlogPostsCount( $userId , $dashboard = true )
	{
		...
		$query	= 'SELECT COUNT(1) FROM ' . $db->nameQuote( '#__easyblog_post' ) . ' '
				. 'WHERE ' . $db->nameQuote( 'source' ) . '=' . $db->Quote( '' ) . ' '
				. 'AND (' . $db->nameQuote( 'created_by' ) . '=' . $db->Quote( $userId ) . ' '
				. 'AND ' . $db->nameQuote( 'published' ) . '!=' . $db->Quote( POST_ID_TRASHED );
