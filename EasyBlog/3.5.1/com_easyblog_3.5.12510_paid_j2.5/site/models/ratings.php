...
class EasyBlogModelRatings extends JModel
{
	/**
	 * Tag total
	 *
	...
	{
		$db		= JFactory::getDBO();
		$query	= 'SELECT COUNT(1) FROM ' . $db->nameQuote( '#__easyblog_ratings' ) . ' '
	...
	{
		$db		= JFactory::getDBO();
		$query	= 'SELECT AVG(value) AS ratings, COUNT(1) AS total FROM ' . $db->nameQuote( '#__easyblog_ratings' ) . ' '
				. 'WHERE ' . $db->nameQuote( 'uid' ) . '=' . $db->Quote( $uid ) . ' '
				. 'AND ' . $db->nameQuote( 'type' ) . '=' . $db->Quote( $type );
