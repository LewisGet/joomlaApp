...
class EasyBlogModelCategory extends JModel
{
	/**
	 * Category total
	 *
	...
	function getCategories($sort = 'latest', $hideEmptyPost = true, $limit = 0 , $inclusion = array() )
	{
		...
		else
		{
			...
			$query	= 'SELECT a.`id` FROM ' . $db->nameQuote( '#__easyblog_category' ) . ' AS a';
			$query	.= ' LEFT JOIN '. $db->nameQuote( '#__easyblog_post' ) . ' AS b';
