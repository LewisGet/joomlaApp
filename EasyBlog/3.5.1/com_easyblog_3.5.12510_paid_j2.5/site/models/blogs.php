...
class EasyBlogModelBlogs extends JModel
{
	...
	function _buildQueryWhere()
	{
		...
		if($my->id == 0)
		{
			$where[] = $db->nameQuote('private') . '=' . $db->Quote(BLOG_PRIVACY_PUBLIC);
		}
	...
	function getTotalPending()
	{
		$db		= $this->getDBO();
		$query	= 'SELECT COUNT(1) AS `total`' .
				  ' FROM ' . $db->nameQuote( '#__easyblog_drafts' ) .
				  ' WHERE ' . $db->nameQuote( 'pending_approval' ) . ' = ' . $db->Quote( '1' );
	...
	function getTotalUnpublished( $uid )
	{
		...
		$query	= 'SELECT COUNT(1) AS `total`' .
				  ' FROM ' . $db->nameQuote( '#__easyblog_post' ) .
				  ' WHERE ' . $db->nameQuote( 'created_by' ) . '=' . $db->Quote( $uid ) .
				  ' AND ' . $db->nameQuote( 'published' ) . '=' . $db->Quote('0');
