...
class EasyBlogMediaManagerImage extends EasyBlogMediaManagerItem
{
	...
	public function __construct( $file , $baseURI , $relativePath = '' , $includeVariation = false , $flatList = false , $absolutePath = false , $isUpload = false )
	{
		$this->file 				= $file;
		...
		if( $flatList )
		{
			...
			$tmp		= str_ireplace( rtrim( $absolutePath , DS ) , '' , dirname( $file ) );

			$this->baseURI	.= str_ireplace( '\\' , '/' , $tmp );;

			$this->relativePath	= str_ireplace( array( '\\' , '/') , DS , $tmp );
		}
	...
	public function inject( &$obj )
	{
		...
		if( ($this->includeVariation || !$this->includeDimension ) && !$this->flatList )
		{
			...
			if( $variations )
			{
				foreach( $variations as $variation )
				{
					...
					{
						$info	= @getimagesize( $baseFolder . DS . $variation );
						$width	= $info[0];
						$height	= $info[1];
					}
					...
					$data				= $media->getItem( $baseFolder . DS . $variation , dirname( $this->getURI() ) , $this->relativePath )->toObject();
		...
		$path 		= JPath::clean( dirname( $this->file ) . DS . $thumbFile );
		...
		$path		= JPath::clean( dirname( $this->file ) . DS . $iconFile );
	...
	public function createVariation( $absoluteURI , $variationName , $width , $height , $variationType = EBLOG_VARIATION_USER_TYPE )
	{
		...
		$path			= dirname( $this->file ) . DS . $variationFile;
		...
		$simpleImage->resizeToFit( $width , $height );

		$state			= $simpleImage->save( $path , $simpleImage->image_type , $config->get( 'main_upload_quality') );
		...
		$media			= new EasyBlogMediaManager();
		$data			= $media->getItem( $path , dirname( dirname( $this->getURI() ) ) , $this->relativePath)->toObject();
	...
	public function getSystemVariations()
	{
		...
		foreach( $variations as $variation )
		{
			$systemVariationFile 	= dirname( $this->file ) . DS . EBLOG_SYSTEM_VARIATION_PREFIX . '_' . $variation . '_' . $fileName;
			...
			{
				$systemVariationFile 	= dirname( $this->file ) . DS . 'thumb_' . $fileName;
			...
			$media			= new EasyBlogMediaManager();
			$data			= $media->getItem( $systemVariationFile , $uri , $this->relativePath )->toObject();
		...
		$data		= $media->getItem( $this->file , dirname( $this->getURI() ) , $this->relativePath)->toObject();
	...
	public static function upload( $storagePath , $fileName , $fileItem )
	{
		...
		$originalStorage	= JPath::clean( $storagePath . DS . $fileName );
