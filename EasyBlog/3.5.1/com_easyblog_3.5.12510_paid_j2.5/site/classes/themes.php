...
class CodeThemes
{
	...
	function CodeThemes( $sel_theme = null )
	{
		...
		$rawParams 		= EBLOG_THEMES . DS . $currentTheme . DS . 'config.xml';
		...
		if( JFile::exists( $rawParams ) && !$this->dashboard )
		{
			...
			$defaultParams 		= EBLOG_THEMES . DS . $currentTheme . DS . 'config.ini';

			if( JFile::exists( $defaultParams ) )
			{
				$this->params->loadINI( JFile::read( $defaultParams ) );
			}
			...
			if( !empty( $themeConfig->params ) )
			{
				...
				{
					$this->params->bind( $themeConfig->params , 'INI' );
				}
				else
				{
					$this->params->bind( $themeConfig->params );
				}
			}
		}
	...
	private function _getThemeConfig( $theme )
	{
		...
		if( empty($loaded[$theme]) )
		{
			...
			else
			{
				...
				$content 	= JFile::read( EBLOG_THEMES . DS . $theme . DS . 'config.ini' );
