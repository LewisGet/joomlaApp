...
class EasyBlogImage
{
	...
	public function __construct( $fileName , $storagePath , $storageURI )
	{
		...
		if( $paths[ 'dirname' ] != '.' )
		{
			$this->storage	.= str_ireplace( array( '/' , '\\' ) , DS , $paths[ 'dirname' ] );
		...
		$this->original	= $this->storage . DS . $this->image;
		...
		$this->sizes[ 'icon' ]->width   = $config->get( 'main_icon_width' );
		$this->sizes[ 'icon' ]->height  = $config->get( 'main_icon_height' );
		$this->sizes[ 'icon' ]->quality = $config->get( 'main_icon_quality' );
	...
	public function initDefaultSizes()
	{
		...
		foreach( $this->sizes as $size => $props )
		{
			...
			if( $size != 'original' )
			{
				...
				$storage	= rtrim( $this->storage , '/\\' ) . DS . $fileName;
	...
	public function getSizes()
	{
		...
		$file 		= JPATH_ROOT . DS . 'templates' . DS . $template . DS . 'html' . DS . 'com_easyblog' . DS . 'image.ini';
		...
		if( !JFile::exists( $file ) )
		{
			...
			$file 		= dirname( $theme->getPath() ) . DS . 'default' . DS . 'image.ini';
			...
			{
				$file 		= $theme->getPath() . DS . 'image.ini';
	...
	public function getSource( $size , $html = false )
	{
		...
		if( !isset( $items[ $index ] ) )
		{
			...
			// Get the storage path.
			$storage	= $this->storage . DS . $fileName;
