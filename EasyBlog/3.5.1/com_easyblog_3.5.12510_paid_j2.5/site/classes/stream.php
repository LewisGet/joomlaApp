...
class EasyBlogStream
{
	...
	private function _buildQuery( $userId, $limit = 0, $startdate = '', $isCnt = false )
	{
		...
		$operation  = '( UNIX_TIMESTAMP( now() ) - UNIX_TIMESTAMP( a.`created`) )';
		            ...
		            foreach( $contextKey as $context )
		            {
						...
						foreach( $verbs as $verb )
						{
						    ...
						    foreach($verbActors as $va)
							{
								...
								elseif( $day > 1 && $day <= 7)
								{
									$dayString		= EasyBlogDateHelper::getDate('-' . $min . ' mins')->toFormat( '%A ' . JText::_( 'COM_EASYBLOG_AT' ) . ' ' . $timeformat);
								}
								else
								{
									$dayString		= EasyBlogDateHelper::getDate('-' . $min . ' mins')->toFormat('%b %d ' . JText::_( 'COM_EASYBLOG_AT' ) . ' ' . $timeformat);
								}
