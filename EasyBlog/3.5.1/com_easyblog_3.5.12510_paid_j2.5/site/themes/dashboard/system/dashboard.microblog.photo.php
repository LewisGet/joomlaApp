...
EasyBlog.ready(function($){
	...
	$( '#microblog-photo button[name=insert-url]' ).bind('click', function(){
		...
		if( photoURL == '' || photoURL == '<?php echo $this->escape( JText::_( 'COM_EASYBLOG_MICROBLOG_PASTE_PHOTO_URL' , true ) );?>')
		{
			$( '#eblog-message' )
				.removeClass( 'error info success' )
				.addClass( 'error' )
				.html( '<?php echo JText::_( 'COM_EASYBLOG_MICROBLOG_PLEASE_ENTER_IMAGE_URL' );?>' )
				.show();
	...
	$( '#microblog-photo #photo-source' ).change(function(){
		var action_url  = '<?php echo JURI::root();?>index.php?option=com_easyblog&namespace=site.views.microblog.uploadPhoto&format=ajax&tmpl=component&<?php echo JUtility::getToken();?>=1',
...
<form id="microblog-photo" name="quick-post" method="post">
	...
	<div class="ui-modfoot clearfix">
		...
		<input type="button" value="<?php echo JText::_('COM_EASYBLOG_CANCEL_BUTTON'); ?>" class="buttons" name="cancel" />
	</div>
</form>
