			...
			<li>
				...
				<div>
        	    	<?php if( $system->config->get( 'layout_dashboard_biography_editor' ) ){ ?>
        	    		<?php echo $editor->display('biography', $profile->getBiography() , '300', '300', '10', '10', array( 'image' ,'readmore' ) ); ?>
        	    	<?php } else { ?>
			...
			<li>
				...
				<?php
					...
					if(!empty($multithemes->availableThemes))
					{
						...
						{
							$options[] = JHTML::_('select.option', $theme, JText::_('COM_EASYBLOG_DASHBOARD_THEMES_THEME_'.$theme));
						}
					}
