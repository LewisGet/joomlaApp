...
class EasyBlogRouter extends JRouter
{
	...
	public static function getLanguageQuery()
	{
		...
		{
			$db			= JFactory::getDBO();
			$langQuery	= ' AND (' . $db->nameQuote( 'language' ) . '=' . $db->Quote( $lang ) . ' OR `language` = '.$db->Quote('*').' )';
		}
	...
	public static function _($url, $xhtml = true, $ssl = null , $search = false )
	{
		...
		if( $routingBehavior == 'currentactive' || $routingBehavior == 'menuitemid')
		{
		...
		else
		{
			switch( $view )
			{
				...
				case 'teamblog':
					...
					if( empty( $exItemid ) )
					{
						$exItemid = EasyBlogRouter::getItemId( 'teamblog' );
					}
	...
	public static function isSh404Enabled()
	{
		...
		if( defined('sh404SEF_AUTOLOADER_LOADED') && JFile::exists(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_sh404sef'.DS.'sh404sef.class.php'))
		{
	...
	public static function getBloggerPermalink( $id )
	{
		...
		if( empty( $bloggerPermalink ) )
		{
			...
			$query	= 'UPDATE `#__easyblog_users` SET `permalink` =' . $db->Quote( $bloggerPermalink ) . ' '
					. 'WHERE ' . $db->nameQuote( 'id' ) . '=' . $db->Quote( $profile->id );
			$db->setQuery( $query );
			$db->query();
		}
	...
	public static function getCategoryPermalink( $id )
	{
		...
		JTable::addIncludePath( EBLOG_TABLES );
		$table	= EasyBlogHelper::getTable( 'ECategory' , 'Table' );
		$table->load( $id );
	...
	public static function getBlogSefPermalink( $id , $external = false )
	{
		...
		if( !isset( $permalinks[ $id ] ) )
		{
			...
			switch( $config->get( 'main_sef' ) )
			{
				// Date based SEF mode
				case 'date':
					$date				= JFactory::getDate( $data->created );
	...
	public static function getRoutedURL( $url , $xhtml = false , $external = false )
	{
		if( !$external )
		{
			return EasyBlogRouter::_( $url , $xhtml );
		}
		...
		$oriURL = $url;
		$url 	= str_replace('/administrator/', '/', EasyBlogRouter::_( $url  , $xhtml, null, $isDashboard ));
		...
		if( $mainframe->isAdmin() && EasyBlogRouter::isSefEnabled() )
		{
			...
			$urls	= $router->build($oriURL)->toString(array('path', 'query', 'fragment'));
		...
		// If this is from admin and sef is not enabled we just use JURI::root() as the user's site may contain a subfolder
		// Using $uri->toString from the back end would not help.
		if( $mainframe->isAdmin() )
		{
			return JURI::root() . ltrim($url , '/');
		}
	...
	public static function _isBlogPermalinkExists( $permalink )
	{
		...
		$query	= 'SELECT COUNT(1) FROM ' . $db->nameQuote( '#__easyblog_post' ) . ' '
				. 'WHERE ' . $db->nameQuote( 'permalink' ) . '=' . $db->Quote( $permalink );
	...
	public static function getItemIdByDashboardLayout( $layout )
	{
		...
		if( !isset( $dashboardLayout[ $layout ] ) )
		{
			...
			$query	= 'SELECT ' . $db->nameQuote('id') . ' FROM ' . $db->nameQuote( '#__menu' ) . ' '
					. 'WHERE ' . $db->nameQuote( 'link' ) . '=' . $db->Quote( 'index.php?option=com_easyblog&view=dashboard&layout=' .$layout) . ' '
					. 'AND ' . $db->nameQuote( 'published' ) . '=' . $db->Quote( '1' )
					. self::getLanguageQuery()
					. ' LIMIT 1';
	...
	public static function getItemIdByTeamBlog( $teamId )
	{
		...
		if( !isset( $teamblogItems[ $teamId ] ) )
		{
			...
			$query	= 'SELECT ' . $db->nameQuote('id') . ' FROM ' . $db->nameQuote( '#__menu' ) . ' '
					. 'WHERE ' . $db->nameQuote( 'link' ) . '=' . $db->Quote( 'index.php?option=com_easyblog&view=teamblog&layout=listings&id='.$teamId ) . ' '
					. 'AND ' . $db->nameQuote( 'published' ) . '=' . $db->Quote( '1' ) 
					. self::getLanguageQuery()
					. ' LIMIT 1';
	...
	public static function getItemIdByCategories( $categoryId )
	{
		...
		if( !isset( $categoryItems[ $categoryId ] ) )
		{
			...
			{
				$query	= 'SELECT ' . $db->nameQuote('id') . ' FROM ' . $db->nameQuote( '#__menu' ) . ' '
						. 'WHERE ' . $db->nameQuote( 'link' ) . '=' . $db->Quote( 'index.php?option=com_easyblog&view=categories&layout=listings&id='.$categoryId) . ' '
						. 'OR ' . $db->nameQuote( 'link' ) . ' LIKE ' . $db->Quote( 'index.php?option=com_easyblog&view=categories&layout=listings&id='.$categoryId . '&limit%') . ' '
						. 'AND ' . $db->nameQuote( 'published' ) . '=' . $db->Quote( '1' ) 
						. self::getLanguageQuery()
						. ' LIMIT 1';
	...
	public static function getItemIdByBlogger( $bloggerId )
	{
		...
		if( !isset( $bloggerItems[ $bloggerId ] ) )
		{
			...
			$query	= 'SELECT ' . $db->nameQuote('id') . ' FROM ' . $db->nameQuote( '#__menu' ) . ' '
					. 'WHERE ' . $db->nameQuote( 'link' ) . '=' . $db->Quote( 'index.php?option=com_easyblog&view=blogger&layout=listings&id='.$bloggerId ) . ' '
					. 'AND ' . $db->nameQuote( 'published' ) . '=' . $db->Quote( '1' ) 
					. self::getLanguageQuery()
					. ' LIMIT 1';
	...
	public static function getItemIdByTag( $tagId )
	{
		...
		if( !isset( $tagItems[ $tagId ] ) )
		{
			...
			$query	= 'SELECT ' . $db->nameQuote('id') . ' FROM ' . $db->nameQuote( '#__menu' ) . ' '
					. 'WHERE ' . $db->nameQuote( 'link' ) . '=' . $db->Quote( 'index.php?option=com_easyblog&view=tags&layout=tag&id='.$tagId) . ' '
					. 'OR ' . $db->nameQuote( 'link' ) . ' LIKE ' . $db->Quote( 'index.php?option=com_easyblog&view=tags&layout=tag&id='.$tagId . '&limit%') . ' '
					. 'AND ' . $db->nameQuote( 'published' ) . '=' . $db->Quote( '1' )
					. self::getLanguageQuery()
					. ' LIMIT 1';
