...
class EasyBlogGroupsHelper
{
	 ...
	 */
	public function getFormHTML( $external, $groupId = '0' , $blogSource = '')
	{
		$contents	= '';
		...
		if( JPluginHelper::isEnabled( 'system' , 'groupeasyblog' ) && $this->testExists( 'jomsocial' ) )
		{
			$contents	= $this->jomsocialForm( $external, $groupId , $blogSource );
		}
	...
	public function testExists( $source )
	{
		...
		{
			case 'jomsocial':
				return JFile::exists( JPATH_ROOT . DS . 'components' . DS . 'com_community' . DS . 'libraries' . DS . 'core.php' );
			break;
		}
	}
	...
	private function jomsocialForm( $external, $groupId = '0' , $blogSource = '' )
	{
		$my		= JFactory::getUser();
	...
	private function sendNotificationsJomsocial( $blog , $isNew , $key , $source , $author )
	{
		...
		if( !class_exists( 'CommunityModelGroups' ) )
		{
			...
			JLoader::import( 'groups' , JPATH_ROOT . DS . 'com_community' . DS . 'models' );
		}
