...
class EasyBlogNotificationHelper
{
	...
	public function getBloggerSubscriberEmails( &$emails = array() , $blog )
	{
		if( !class_exists( 'EasyBlogModelBlogger' ) )
		{
			...
			JLoader::import( 'blogger' , JPATH_ROOT . DS . 'components' . DS . 'com_easyblog' . DS . 'models' );
		}
		$model 			= JModel::getInstance( 'Blogger' , 'EasyBlogModel' );
	...
	public function send( $emails , $emailTitle , $template , $data )
	{
		...
		$defaultEmailFrom  	= ( EasyBlogHelper::getJoomlaVersion() >= '1.6' ) ? $jConfig->get( 'mailfrom') : $jConfig->getValue( 'mailfrom');
		$defaultFromName  	= ( EasyBlogHelper::getJoomlaVersion() >= '1.6' ) ? $jConfig->get( 'fromname') : $jConfig->getValue( 'fromname');
