...
class EasyBlogHelper
{
	...
	public static function getPagination($total, $limitstart, $limit, $prefix = '')
	{
		...
		{
			require_once( EBLOG_CLASSES . DS . 'pagination.php' );
	...
	public static function formatMicroblog( &$row )
	{
		...
		$path		= EBLOG_CLASSES . DS . 'microblog' . DS . strtolower( $row->source ) . '.php';
		...
		require_once( EBLOG_CLASSES . DS . 'microblog' . DS . strtolower( $row->source ) . '.php' );
	...
	public static function formatBlog( $data , $loadComments = false , $removeFeaturedImage = true, $loadVideo = true , $frontpage = false )
	{
		...
		for($i = 0; $i < count($data); $i++)
		{
			...
			EasyBlogHelper::truncateContent( $blog , $loadVideo , $frontpage );
	 ...
	 * @access	public
	 */
	public static function truncateContent( &$row , $loadVideo = false , $frontpage = false )
	{
		$config			= EasyBlogHelper::getConfig();
		$truncate		= true;
		...
		switch( $config->get( 'main_truncate_type' ) )
		{
			...
			default:
				...
				if( $frontpage && $config->get( 'main_truncate_media_position' ) == 'hidden' )
				{
				...
				if( count( $matches ) > $maxTag )
				{
					...
					preg_match_all( '/\<div/i' , strtolower( $row->text ), $totalOpenDiv );

					if( isset( $totalOpenDiv[0] ) && !empty( $totalOpenDiv[0] ) )
					{
						$totalOpenDiv	= count( $totalOpenDiv[0] );

						preg_match_all( '/\<\/div\>/is' , strtolower( $row->text ) , $totalClosedDiv );

						$totalClosedDiv	= count( $totalClosedDiv[0] );

						$totalDivToAdd	= $totalOpenDiv - $totalClosedDiv;

						if( $totalDivToAdd > 0 )
						{
							for( $y = 1; $y <= $totalDivToAdd; $y++ )
							{
								$row->text 	.= '</div>';
							}
						}
		...
		if( $config->get( 'main_truncate_ellipses' ) && $row->readmore )
		{
		...
		if( !$row->source || empty( $row->source ) )
		{
			...
			if(  $config->get( 'main_truncate_type' ) == 'chars' ||  $config->get( 'main_truncate_type' ) == 'words' )
			{
				// Append image in the post if truncation is done by characters
				if( ($config->get( 'main_teaser_image' ) && !$frontpage ) || ( $frontpage && $config->get('main_truncate_media_position') != 'hidden' ) )
				{
					...
					if( !empty( $matches ) )
					{
						...
						{
							$row->text	= '<div class="teaser-image clearfix ' . $config->get( 'main_teaser_image_align' ) . '" style="margin:8px;max-width:98%;">' . $matches[ 0 ] . '</div>' . $row->text;
						}
						else
						{
							$row->text	.= '<div class="teaser-image clearfix" style="margin:8px;max-width:98%;text-align: ' . $config->get( 'main_teaser_image_align' ) . ' !important;">' . $matches[ 0 ] . '</div>';
						}
					}
				}
			}

			// @task: Prepend the gallery items in the start of the blog posts.
			if( $config->get( 'main_truncate_media_position') == 'top' )
			{
				$row->text	= $embedHTML . $row->text;
			}

			// @task: Append the gallery items at the end of the blog post.
			// @task: Prepend the gallery items in the start of the blog posts.
			if( $config->get( 'main_truncate_media_position') == 'bottom' )
			{
				$row->text	.= $embedHTML;
			}
		}
			...
			if($config->get('integrations_jomsocial_submit_content'))
			{
				...
				else
				{
					...
					if( $blog->getImage() )
					{
						...
						if( $imageSource )
						{
							$blogContent	= '<a href="' . $blogLink . '"><img src="' . $imageSource . '" style="margin: 0 5px 5px 0;float: left; height: auto; width: 120px !important;"/></a>' . $blogContent . '<div style="clear: both;"></div>';
						}
					}
	...
	public static function uploadAvatar( $profile, $isFromBackend = false )
	{
		...
		if (isset($file['name']))
		{
			...
			require_once( EBLOG_HELPERS . DS . 'image.php' );
			require_once( EBLOG_CLASSES . DS . 'easysimpleimage.php' );
	...
	public static function processScheduledPost( $max = 5 )
	{
		...
		foreach($blogs as $item)
		{
			...
			$blog->published	= 1;
			$blog->isnew		= ( $blog->isnew ) ? true : false ;
...
/*]]>*/";
	...
	{
		require_once( EBLOG_HELPERS . DS . 'socialbutton.php' );
	...
	public static function getThemeObject( $name )
	{
		...
		if( !JFile::exists( $path . DS . $name . DS . $file ) )
		{
			return false;
		}

		$manifest	= JFile::read( $path . DS . $name . DS . $file );
		$parser		= JFactory::getXMLParser('Simple');

		// Read the manifest file
		$parser->loadString( $manifest );

		$obj			= new stdClass();
		$obj->name		= $name;
		$obj->path		= $path . DS . $name;
		$obj->writable	= is_writable( $path . DS . $name );
	...
	{
		require_once( EBLOG_CLASSES . DS . 'stream.php' );
	...
	public function getCategoryInclusion( $categories )
	{
