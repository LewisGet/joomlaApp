...
class EasyBlogGalleryHelper
{
	...
	private static function parseJSON( $jsonString , $userId = '' )
	{
		...
		if( isset( $obj->place ) )
		{
			...
			if( $place == 'shared' )
			{
				$storage	= JPATH_ROOT . DS . trim( $cfg->get( 'main_shared_path' ) , '/\\' ) . DS . $folder;
			...
			else
			{
				...
				$storage		= JPATH_ROOT . DS . trim( $cfg->get( 'main_image_path' ) , '/\\' ) . DS . $place[1] . DS . $folder;
		...
		{
			// Legacy method.
			$tmpStorage	= str_ireplace( '/' , DS , rtrim( $cfg->get( 'main_image_path' ) , '/' ) );
			$url		= rtrim( JURI::root() , '/' ) . '/' . str_ireplace( '\\' , '/' , $tmpStorage ) . '/' . $userId . '/' . $obj->path;
			$storage	= JPATH_ROOT . DS . $tmpStorage . DS . $userId . DS . $obj->path;
		}

		// @task: Replace all / and \ from storage to the directory structure.
		$storage        = str_ireplace( array( '\\'  , '/' ) , DS , $storage );
		...
		foreach( $items as $item )
		{
			...
			if( !preg_match( $regexFilter , $item ) )
			{
				...
				$itemPath		= $storage . DS . $item;

				// @TODO: Currently the thumbnail variation is hardcoded. Perhaps make this configurable.
				$itemThumbPath	= $storage . DS . EBLOG_SYSTEM_VARIATION_PREFIX . '_thumbnail_' . $item;
				...
				if( JFile::exists( $itemPath ) )
				{
					...
					if( !JFile::exists( $itemThumbPath ) )
					{
						...
						$itemThumbPath		= $storage . DS . EBLOG_MEDIA_THUMBNAIL_PREFIX . $item;
