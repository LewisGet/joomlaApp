...
class EasyBlogVideosHelper
{
	...
	*/
		...
		if( !empty( $matches ) )
		{
			foreach( $matches as $match )
			{
				...
				switch( $type )
				{
					...
					case 'videolink':
						$content	= $this->processExternalVideos( $content , $isPlain , $search );
					break;
	...
	public function processInternalVideos( $content , $isPlain = false , $findText = '' , $result = '' )
	{
		...
		if( !empty($matches) && isset( $matches[0 ] ) )
		{
			...
			if( isset( $data[1] ) )
			{
				...
				$storage	= str_ireplace( '/' , DS , rtrim( $cfg->get( 'main_image_path' ) , '/' ) );
	 ...
	 *
	 */
	public function processExternalVideos( $content , $isPlain = false , $findText = '' )
	{
		$cfg		= EasyBlogHelper::getConfig();
