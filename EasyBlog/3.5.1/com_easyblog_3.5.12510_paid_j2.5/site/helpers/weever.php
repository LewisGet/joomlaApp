...
class EasyBlogWeeverItem
{
	...
	public function setTags( &$item )
	{
		...
		if( $tags )
		{
			foreach( $tags as $tag )
			{
				$data			= array();
				$data[ 'name' ]	= $tag->title;
				$data[ 'link' ]	= EasyBlogRouter::getRoutedUrl( 'index.php?option=com_easyblog&view=tag&layout=listings&format=weever&id=' . $tag->id , false , true );
	...
	public function map( &$blog )
	{
		...
		// @task: Set the URL
		$this->url		= EasyBlogRouter::getRoutedUrl( 'index.php?option=com_easyblog&view=entry&id=' . $blog->id . '&format=weever' , false , true );
