...
class EasyBlogViewCategories extends EasyBlogView
{
	...
	function display( $tmpl = null )
	{
		...
		$data				= $categoryModel->getCategories($sort, $config->get( 'main_categories_hideempty' ) , 0 , $inclusion );
		...
		if(!empty($data))
		{
			for($i = 0; $i < count($data); $i++)
			{
				...
				if (count($row->childs) > $initialLimit)
				{
					...
					$nestedLinks .= '<script type="text/javascript>EasyBlog.ready(function($){ $(".more-subcategories-toggle a").click(function(){ $(this).parent().hide().next(".more-subcategories").show(); }); });</script>';
					$nestedLinks .= '<span class="more-subcategories-toggle"> ' . JText::_('COM_EASYBLOG_AND') . ' <a href="javascript: void(0);">' . JText::sprintf('COM_EASYBLOG_OTHER_SUBCATEGORIES', count($row->childs) - $initialLimit) . '</a></span>';
		...
		// @task: Set page title
		parent::setPageTitle( JText::_('COM_EASYBLOG_CATEGORIES_PAGE_TITLE') , $pagination , true );
...
// 		exit;
	...
	function listings()
	{
		...
		if (count($category->childs) > $initialLimit)
		{
			...
			$nestedLinks .= '<span class="more-subcategories-toggle"> ' . JText::_('COM_EASYBLOG_AND') . ' <a href="javascript: void(0);">' . JText::sprintf('COM_EASYBLOG_OTHER_SUBCATEGORIES', count($category->childs) - $initialLimit) . '</a></span>';
		...
		$title					= EasyBlogHelper::getPageTitle( $category->title );
