...
class EasyBlogViewDashboard extends EasyBlogView
{
	...
	private function _toggle( $id , $ajax , $action = '' )
	{
		...
		$message	= $blog->published ? JText::_( 'COM_EASYBLOG_DASHBOARD_ENTRIES_PUBLISHED_SUCCESS' ) : JText::_( 'COM_EASYBLOG_DASHBOARD_ENTRIES_UNPUBLISHED_SUCCESS' );
    ...
    function checkPublishStatus($status, $unpublishDate)
    {
		...
		$unpublishing 	= JFactory::getDate( empty($unpublishDate) ? '' : $unpublishDate , $txOffset);
	...
	function saveDraft( $params , $content , $intro )
	{
		...
		$date	= EasyBlogDateHelper::dateWithOffSet( JFactory::getDate()->toMySQL() );
