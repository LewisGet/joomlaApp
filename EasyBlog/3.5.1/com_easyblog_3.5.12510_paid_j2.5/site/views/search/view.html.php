...
class EasyBlogViewSearch extends EasyBlogView
{
	...
	{		
		...
		$document->setTitle( EasyBlogHelper::getPageTitle( JText::_( 'COM_EASYBLOG_SEACH_PAGE_TITLE' ) ) );
		...
		if( count($posts) > 0 )
		{
			...
			for($i = 0; $i < count($posts); $i++ )
			{
				...
				$row->content 	= preg_replace( $pattern , '<span class="search-highlight">\0</span>' , strip_tags( $row->content ) );
			}
		}
