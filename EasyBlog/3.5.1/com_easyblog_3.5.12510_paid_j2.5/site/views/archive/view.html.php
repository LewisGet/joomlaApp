...
class EasyBlogViewArchive extends EasyBlogView
{
	function calendar( $tmpl = null )
	{
		...
		$displayListing	= $menuParams->get( 'archivelist' );

		if( $displayListing && empty( $archiveYear ) && empty( $archiveMonth ) && empty( $archiveDay ) )
		{
		...
		$viewDate		= JFactory::getDate( $archiveYear . '-' . $archiveMonth . '-' . $archiveDay);
	...
	public function display()
	{
		...
		$data			= $model->getBlogsBy( '' , 0 , 'latest' );
		$pagination		= $model->getPagination();
