...
class EasyBlogViewMedia extends EasyBlogView
{
	...
	public function createFolder()
	{
		...
		$destination	= $absolutePath . DS .'index.html';
		...
		$obj 	= $media->getItem( $absolutePath , $uri , $relative )->toArray();
	...
	public function listItems()
	{
		...
		$items 		= $media->getItem( $absolutePath , $absoluteURI , $source , $variation, $place )->toArray();
	...
	/**
	 * Responsible to only return a flat list of images for the caller.
	 *
	 */
	public function listImages()
	{
		...
		if( !$media->exists( $absolutePath ) || !$absolutePath )
		{
			// @task: Create the folder for the user. There could be a possibility that the folder hasn't been created yet.
			JFolder::create( $absolutePath );

			// @task: Copy the initial index.html file over.
			JFile::copy( EBLOG_ROOT . DS . 'index.html' , $absolutePath . DS . 'index.html' );
		}

		$result 		= $media->getItems( $absolutePath , $absoluteURI , '' , false , true )->toArray();
