...
class EasyBlogViewMicroBlog extends EasyBlogView
{
	public function save()
	{
		...
		$blog->created 		= JFactory::getDate()->toMySQL();
	...
	{
		$path		= EBLOG_CLASSES . DS . 'microblog' . DS . strtolower( $type ) . '.php';
		...
		require_once( EBLOG_CLASSES . DS . 'microblog' . DS . strtolower( $type ) . '.php' );
	...
	public function uploadPhoto()
	{
		...
		$userUploadPath    	= JPATH_ROOT . DS . str_replace('/', DS, $imagePath . DS . $my->id);
		...
		$storagePath 	= JPath::clean( $storageFolder . DS . $filename );
...
// 			{
// 				$tmpName	= $i . '_' . JFactory::getDate()->toFormat( "%Y%m%d-%H%M%S" ) . '_' . $filename;
// 				$storagePath	= JPath::clean( $storageFolder . DS . $tmpName );
// 				$i++;
// 			}
// 			$filename	= $tmpName;
// 		}
...
// 		// @task: Ensure that images goes through the same resizing format when uploading via media manager.
		...
		$uploaded 			= $media->upload( $storageFolder , $imageURI , $file , '/' );
