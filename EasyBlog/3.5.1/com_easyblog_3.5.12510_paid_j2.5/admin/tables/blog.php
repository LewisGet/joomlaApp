...
class TableBlog extends JTable
{
	...
	{
		$db		= JFactory::getDBO();
		$query	= 'DELETE FROM ' . $db->nameQuote( '#__easyblog_reports' );
		$query	.= ' WHERE ' . $db->nameQuote( 'obj_id' ) . '=' . $db->Quote( $this->id );
		$query	.= ' AND ' . $db->nameQuote( 'obj_type' ) . '=' . $db->Quote( EBLOG_REPORTING_POST );
	...
	public function getBlogContribution()
	{
		...
		$query	= 'SELECT * FROM ' . $db->nameQuote( '#__easyblog_external' ) . ' '
				. 'WHERE ' . $db->nameQuote( 'post_id' ) . '=' . $db->Quote( $this->id );
		$db->setQuery( $query );
	...
	public function store( $log = true )
	{
		...
		if( $source == 'easyblog' && $state && $this->published == POST_ID_PUBLISHED && $isNew && $log )
		{
			...
			if( $my->id == $this->created_by && EasyBlogHelper::isAUPEnabled() )
			{
				...
				AlphaUserPointsHelper::newpoints( 'plgaup_easyblog_add_blog', '', 'easyblog_add_blog_' . $this->id, JText::sprintf('COM_EASYBLOG_AUP_NEW_BLOG_CREATED', $url, $this->title) );
			}
	...
	public function processTrackbacks()
	{
		...
		{
			JLoader::import( 'trackbacksent' , EBLOG_ROOT . DS . 'models' );
		}
	...
	public function processTags( $tags , $isNew = true )
	{
		...
		{
			JLoader::import( 'posttag' , EBLOG_ROOT . DS . 'models' );
		}
		...
		if( $isNew )
		{
			...
			if( !empty( $defaultTags ) )
			{
				foreach( $defaultTags as $key => $tagId )
				{
					...
					$postTagModel->add( $tagId , $this->id , JFactory::getDate()->toMySQL() );
				}
			}
		}
		...
		if( !empty( $tags ) )
		{
			...
			foreach( $tags as $title )
			{
				...
				if( !$exists )
				{
					...
					$table->set( 'created'		, JFactory::getDate()->toMySQL() );
				...
				$postTagModel->add( $table->id , $this->id , JFactory::getDate()->toMySQL() );
			}
		}
	}
	...
	public function saveTags( $tags )
	{
		...
		foreach( $tags as $title )
		{
			...
			if( !$tag->exists( $title ) && $acl->rules->create_tag )
			{
				...
				$tag->title 		= $title;
				$tag->created 		= JFactory::getDate()->toMySQL();
				$tag->store();
			}

			// Add the association for the tag.
			$model->add( $tag->id , $blog->id , JFactory::getDate()->toMySQL() );
		}
	...
	public function getImage()
	{
		...
		if( !isset( $image[ $this->id ] ) )
		{
			...
			{
				$storagePath	= JPATH_ROOT . DS . trim( $cfg->get( 'main_shared_path' ) , '/\\');
			...
			else
			{
				...
				$storagePath	= JPATH_ROOT . DS . trim( $cfg->get( 'main_image_path' ) , '/\\') . DS . $place[1];
			...
			$itemPath 			= $storagePath . DS . trim( $imageObject->path , '/\\' );
