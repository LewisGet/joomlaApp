...
class EasyBlogControllerBlogs extends EasyBlogController
{
	...
	function approveBlog()
	{
		...
		if( !$blog->store() )
		{
			...
			$app->redirect( EasyBlogRouter::_( 'index.php?option=com_easyblog&view=pending' , false) );
		}
	...
	function _saveDraft( )
	{
		...
		if( $draft->store() )
		{
			...
			if( $saveNew )
			{
				$mainframe->redirect( JRoute::_('index.php?option=com_easyblog&view=blog', false) );
				return;
			}
	...
	function savePublish()
	{
		...
		if( $saveNew )
		{
			$this->setRedirect( JRoute::_('index.php?option=com_easyblog&view=blog', false) );
			return;
		}

		if( $this->getTask() == 'saveApply' )
		{
			$this->setRedirect( JRoute::_('index.php?option=com_easyblog&view=blog&blogid=' . $blog->id , false) );
			return;
		}
	...
	public function trash()
	{
		...
		$this->setRedirect( 'index.php?option=com_easyblog&view=blogs' , $message , 'success' );
	}
