...
class J2StoreModelProductFiles extends JModelList
{
	...
	*/
	...
	function saveFile($file){
		...
		if(!$id){
			$this->setError(JText::_('You should save article first before uploading files'));
			return false;
		}
		...
		if( !JFolder::exists($folder_base_path)){
			$this->setError(JText::_('Please set the correct path to upload files in J2Store params'));
			return false;
		}
			
		//if exists lets create products folder here
		$products_folder_path  = $folder_base_path.DS.'products';
		...
		$folder_path = $products_folder_path.DS.$id;
	...
	function getFiles(){
		...
		if( ! JFolder::exists($folder_path)){
			...
			$this->setError(JText::_('No files found '));
			return false;
		}
	...
	function deleteFile($file_name){
		...
		if(JFile::exists($file_name) ) {
			...
			$this->setError(JText::_('Error in deleting the file'));
		}
		return false;
	}
